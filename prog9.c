...
void addpoly(node *head1,node *head2,node *head3)
{
	...
	while(p1!=head1)
	{
		if(p1->flag==0)
		{
			insertend(head3,p1->coeff,p1->expo1,p1->expo2,p1->expo3);
		}
		p1=p1->next;
	}
	while(p2!=head2)
	{
		if(p2->flag==0)
		{
			insertend(head3,p2->coeff,p2->expo1,p2->expo2,p2->expo3);
		}
		p2=p2->next;`
	}
}
void display(node *head)
{
	...
	while(p1->next!=head)
	{
		printf("%dx^%dy^%dz^%d+",p1->coeff,p1->expo1,p1->expo2,p1->expo3);
		p1=p1->next;
	}
	printf("%dx^%dy^%dz^%d",p1->coeff,p1->expo1,p1->expo2,p1->expo3);
}
